...
function getPublicModules(modules, ns) {
	var publicModules = [];
	modules.forEach(function(module) {
		if (module.isPublic) {
			if (ns) {
				module = utils.extend({}, module, { id: [ns, module.id].join('/') });
			}
...
function parseModules(options) {
	...
	utils.findFiles(options.from, options.baseDir).forEach(function(filePath) {
...
function compileMinified(modules, options) {
	...
	if (options.outputMinified) {
		...
		exposeModules(toplevel.body, getPublicModules(modules, options.rootNS));
		...
		currentReporter.info("Writing minified version output to: " + utils.toUnixPath(options.outputMinified));
...
function compileSource(modules, options) {
	...
	if (options.outputSource) {
		...
		var publicModules = getPublicModules(modules, options.rootNS);
		...
		currentReporter.info("Writing source version output to: " + utils.toUnixPath(options.outputSource));
...
function compileDevelopment(modules, options) {
	...
	if (options.outputDev) {
		...
		currentReporter.info("Writing development version to: " + utils.toUnixPath(options.outputDev));
...
function compile(options) {
	...
	if (options.verbose) {
		...
		options.reporter.level = "info";
	}

	currentReporter = new reporter.create(options.reporter);
